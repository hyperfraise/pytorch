...
c10::intrusive_ptr<Conv3dOpContext> unpack_3d(
    Tensor&& weight,
    c10::optional<Tensor>&& bias,
    std::vector<int64_t>&& stride,
    std::vector<int64_t>&& padding,
  ...
  memcpy(packedWeight.data_ptr(), packed_buffer.data(), size_bytes);
  return c10::make_intrusive<Conv3dOpContext>(
      std::move(packedWeight),
      std::move(bias),
      stride,
      padding,
...
TORCH_LIBRARY_3D(metal, m) {
  m.class_<Conv3dOpContext>("Conv3dOpContext")
      .def_pickle(
          [](const c10::intrusive_ptr<Conv3dOpContext>& op_context)
              -> SerializationTypeConv3dPrePack { // __getstate__
            return op_context->pack();
          },
          [](SerializationTypeConv3dPrePack state)
              -> c10::intrusive_ptr<Conv3dOpContext> { // __setstate__
            return unpack_3d(
                std::move(std::get<0>(state)),
                std::move(std::get<1>(state)),
                std::move(std::get<2>(state)),
...
TORCH_LIBRARY_3D(metal_prepack, m) {
  m.def(
      "conv3d_prepack(Tensor W, Tensor? B, int[3] stride, "
      "int[3] padding, int[3] dilation, int groups, "
      "Scalar? output_min=None, Scalar? output_max=None) "
      "-> __torch__.torch.classes.metal.Conv3dOpContext");
  m.def(
      "conv3d_run(Tensor X, "
      "__torch__.torch.classes.metal.Conv3dOpContext W_prepack) -> Tensor Y");
}

c10::intrusive_ptr<Conv3dOpContext> conv3d_prepack(
    Tensor&& weight,
    c10::optional<Tensor>&& bias,
    std::vector<int64_t>&& stride,
    std::vector<int64_t>&& padding,
...
Tensor conv3d_prepack_run(
    const Tensor& input,
    const c10::intrusive_ptr<Conv3dOpContext>& op_context) {
#if (C10_IOS || TARGET_OS_MAC)
  return mpscnn::conv3d(input, *op_context);
#else
  TORCH_CHECK(false, "conv3d_prepack_run can only be invoked on iOS and MacOS");
  return input;
#endif
}
...
TORCH_LIBRARY_IMPL(metal_prepack, CPU, m) {
  m.impl("conv3d_prepack", TORCH_FN(conv3d_prepack));
}

TORCH_LIBRARY_IMPL(metal_prepack, Metal, m) {
  m.impl("conv3d_run", conv3d_prepack_run);
}
