...
class TestQuantizeFx(QuantizationTestCase):
    ...
    def test_attention(self):
        ...
        qconfig_dict = {
            "": None,
            "object_type": [(nn.Conv2d, default_qconfig), ("chunk", None)],
        }
        # make sure it runs
        m = prepare_fx(m, qconfig_dict)
        m(tensor_input)
            ...
            if is_name:
                ...
                prepare_config = {
                    "standalone_module_class": [
                        (StandaloneModule, None, interface_config)
                    ]
                }
    ...
    def test_standalone_module_float_interface(self):
        ...
        prepare_count_check = {ns.call_module(torch.quantization.MinMaxObserver): 2}
...
class TestQuantizeFxOps(QuantizationTestCase):
    ...
    def test_linear_module(self):
        ...
        options = itertools.product(
            [(ModuleLinear(has_relu=False), True)], self.all_quant_types
        )
        quantized_nodes = {
            # is_module
            True: {
    ...
    def test_conv2d_functional(self):
        for bias in [True, False]:
            ...
            expected_node_occurrence = {ns.call_function(torch.ops.quantized.conv2d): 1}
            self.checkGraphModeFxOp(
                conv,
                (torch.randn(4, 1, 4, 4),),
                QuantType.STATIC,
    ...
    def _test_quantized_binary_op_relu_impl(self, binary_op, ibinary_op, quantized_op):
        class OpRelu(torch.nn.Module):
            def __init__(self, is_inplace, is_functional_relu, is_scalar):
                ...
                self.is_scalar = is_scalar
                self.relu = F.relu if self.is_functional_relu else torch.nn.ReLU()
    ...
    def test_qbatch_norm_relu(self):
        ...
        for quant_type, dim in options:
            for instance in [
                ...
                self.checkGraphModeFxOp(
                    instance, self.img_data_dict[dim], quant_type, quantized_nodes[dim]
                )
    ...
    def _test_activation_impl(
        ...
        options = itertools.product(
            [True, False], [True, False], self.static_quant_types
        )
        quantized_nodes = {
            # is_module
    ...
    def test_embedding_bag(self):
        ...
        for dtype in [torch.quint8, torch.quint4x2]:
            ...
            float_qparams_qconfig = QConfigDynamic(
...
class TestQuantizeFxModels(QuantizationTestCase):
    ...
    def test_torchvision(self):
        ...
        quantized_not_working = [("qat", "inception_v3"), ("static", "inception_v3")]

        fx_eager_not_matching = [
            "googlenet",  # because _transform_input is not quantized in eager
            "mobilenet_v2",
        ...
        for mode, name in options:
            ...
            if name in quantized_model_list:
                if (mode, name) in quantized_not_working:
                    ...
                    eager_quantizable_model = (
                        quantized_models.__dict__[name](pretrained=True, quantize=False)
                        .eval()
                        .float()
                    )
