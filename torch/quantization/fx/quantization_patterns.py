...
class Add(QuantizeHandler):
    def __init__(self, quantizer: QuantizerCls, node: Node):
        ...
        self.add_node = node
        self.num_node_args = len(
            [a for a in self.add_node.args[:2] if isinstance(a, Node)]
        )
    ...
    def convert(
        ...
        if self.num_node_args == 1:
            ...
            return quantizer.quantized_graph.create_node(
                "call_function",
                op,
                load_arg(quantized=[quantized_index])(self.add_node.args),
            ...
            return quantizer.quantized_graph.create_node(
                "call_function",
                op,
                load_arg(quantized=True)(self.add_node.args),
                kwargs,
            )
...
class Mul(QuantizeHandler):
    def __init__(self, quantizer: QuantizerCls, node: Node):
        ...
        self.mul_node = node
        self.num_node_args = len(
            [a for a in self.mul_node.args[:2] if isinstance(a, Node)]
        )
    ...
    def convert(
        ...
        if self.num_node_args == 1:
            ...
            return quantizer.quantized_graph.create_node(
                "call_function",
                op,
                load_arg(quantized=[quantized_index])(self.mul_node.args),
                self.mul_node.kwargs,
            )
        else:
            activation_post_process = quantizer.activation_post_process_map[node.name]
            scale, zero_point = activation_post_process.calculate_qparams()
            ...
            return quantizer.quantized_graph.create_node(
                "call_function",
                op,
                load_arg(quantized=True)(self.mul_node.args),
                kwargs,
            )
...
class ConvRelu(QuantizeHandler):
    ...
    def convert(
        ...
        if self.conv_node.op == "call_module":
            ...
            assert self.relu_node is None, (
                "conv module and relu fusion is not executed, "
            ...
            self.conv.activation_post_process = quantizer.activation_post_process_map[
                node.name
            ]
            ...
            if debug:
                ...
                return quantize_node(
                    ...
                    conv_out,
                    quantizer.activation_post_process_map[self.conv_node.name],
                )
            else:
                assert (
                ...
                activation_post_process = quantizer.activation_post_process_map[
                    self.conv_node.name
                ]
...
class LinearReLUQuantizeHandler(QuantizeHandler):
    ...
    def convert(
        ...
        if self.linear_node.op == "call_module":
            # note that relu should already be fused into conv module in the fusion step
            assert self.relu_node is None, (
                "linear module and relu fusion is not executed, "
                "please make sure to run fusion before prepare"
            )
            ...
            if node.name in quantizer.activation_post_process_map:
                ...
                output_activation_post_process = quantizer.activation_post_process_map[
                    node.name
                ]
            else:
            ...
            return quantizer.quantized_graph.create_node(
                ...
                (
                    load_arg(quantized=activation_statically_quantized)(
                        self.linear_node.args[0]
                    ),
                ),
                {},
            )
            ...
            if debug:
                ...
                linear_weight = load_arg(quantized=weight_quantized)(
                    self.linear_node.args[1]
                )
                ...
                if activation_statically_quantized:
                    ...
                    activation_post_process = quantizer.activation_post_process_map[
                        self.linear_node.name
                    ]
                    scale, zero_point, _ = get_per_tensor_qparams(
...
class BatchNorm(QuantizeHandler):
    ...
    def convert(
        ...
        # 1. attach activation post process to module
        self.bn.activation_post_process = quantizer.activation_post_process_map[
            node.name
        ]
        qbn_cls = get_static_quant_module_class(
            type(self.bn), additional_static_quant_mapping
        )
...
class DefaultNode(QuantizeHandler):
    ...
    def convert(
        ...
        if not self.all_node_args:
            return NotImplemented
        assert node.op in ["call_module", "call_function"], (
            "Only call_module and " + "call_function are handled in DefaultNode"
        )
        if convert_custom_config_dict is None:
            convert_custom_config_dict = {}
        additional_static_quant_mapping = convert_custom_config_dict.get("static", {})
...
@register_quant_pattern(
    torch.nn.functional.hardsigmoid, default_affine_fixed_qparams_fake_quant
)
...
class StandaloneModuleQuantizeHandler(QuantizeHandler):
    ...
    def convert(
        ...
        input_quantized_idxs = (
            observed_standalone_module._standalone_module_input_quantized_idxs.tolist()
        )
        ...
        return quantizer.quantized_graph.node_copy(
            node, load_arg(quantized=input_quantized_idxs)
        )
